...
typedef vector<tuple<unsigned int, unsigned int, unsigned int > > RELATION_TYPE;
...
struct sketch_unit {
  ...
  double estimate(int param) const {
    bool success = false;
    double p = 1.0;
    for (unsigned int i = 0; i < table_12.size(); ++i) {
      if (tuple_filter(table_12[i], param)) {
...
int main(int argc, char* argv[]) {
  ...
  for (const auto& graph_ab : graph) {
    ...
    for (auto b_it = b_it_begin; b_it != b_it_end; ++b_it) {
      auto c = get<1>(*b_it);
      data.emplace_back(make_tuple(a, b, c));
    }
  }
