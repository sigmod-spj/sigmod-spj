...
typedef tuple<unsigned int, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int>  TUPLE_12345_TYPE;
...
typedef vector<tuple<unsigned int, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int> > TABLE_12345_TYPE;
typedef vector<tuple<unsigned int, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int> > RELATION_TYPE;
...
struct sketch_unit {
  ...
  TABLE_1234_TYPE table_1234;
  TABLE_12345_TYPE table_12345;
  RELATION_TYPE table_123456;
  ...
  double estimate(int param) const {
    bool success = false;
    double p = 1.0;
    for (unsigned int i = 0; i < table_123456.size(); ++i) {
      if (tuple_filter(table_123456[i], param)) {
  ...
  unsigned int commit_table_1234() {
    if (was_complete) {
      prob = temp_prob;
      temp_prob.clear();
    } else if (current_freq > max_freq) {
      was_complete = true;
      prob = vector<double>(table_1234.size(), max_freq / static_cast<double>(current_freq));
    }
    table_123.clear();
    current_freq = 0;
    return table_1234.size();
  }
  unsigned int commit_table_12345() {
    if (was_complete) {
      prob = temp_prob;
      temp_prob.clear();
    } else if (current_freq > max_freq) {
      was_complete = true;
      prob = vector<double>(table_12345.size(), max_freq / static_cast<double>(current_freq));
    }
    table_1234.clear();
    current_freq = 0;p
    return table_12345.size();
  }
  unsigned int commit_table_123456() {
    if (was_complete) {
      prob = temp_prob;
      temp_prob.clear();
    } else if (current_freq > max_freq) {
      was_complete = true;
      prob = vector<double>(table_123456.size(), max_freq / static_cast<double>(current_freq));
    }
    table_12345.clear();
    return table_123456.size();
  }
  ...
  void add_table_1234(const TUPLE_123_TYPE &sd,
                      const TUPLE_TYPE &i) {
    if (current_freq < max_freq) {
      table_1234.emplace_back
        (make_tuple(get<0>(sd), get<1>(sd), get<2>(sd), get<3>(sd), get<1>(i)));
    } else {
      unsigned int idx = rand() % current_freq;
      if (idx < max_freq) {
        table_1234[idx] =
  ...
  void add_table_12345(const TUPLE_1234_TYPE &sd,
                       const TUPLE_TYPE &i) {
    if (current_freq < max_freq) {
      table_12345.emplace_back
        (make_tuple(get<0>(sd), get<1>(sd), get<2>(sd), get<3>(sd),
                    get<4>(sd), get<1>(i)));
    } else {
      unsigned int idx = rand() % current_freq;
      if (idx < max_freq) {
        table_12345[idx] =
          make_tuple(get<0>(sd), get<1>(sd), get<2>(sd), get<3>(sd), get<4>(sd), get<1>(i));
      }
    }
    current_freq += 1;
  }
  void add_table_123456(const TUPLE_12345_TYPE &sd,
                        const TUPLE_TYPE &i) {
    if (current_freq < max_freq) {
      table_123456.emplace_back
        (make_tuple(get<0>(sd), get<1>(sd), get<2>(sd), get<3>(sd),
                    get<4>(sd), get<5>(sd), get<1>(i)));
    } else {
      unsigned int idx = rand() % current_freq;
      if (idx < max_freq) {
        table_123456[idx] =
...
ostream& operator<<(ostream &os, sketch_unit s) {
  os << "MAX FREQUENCY: " << s.max_freq << endl
     ...
     << "TABLE 12345 SIZE: " << s.table_12345.size() << endl
     << "PROB: ";
...
int main(int argc, char* argv[]) {
  ...
  TABLE_TYPE graph = load_graph((argc>1)?argv[1]:"../data/real/p2p-Gnutella04.txt");//"../data/real/p2p-Gnutella04.txt");
  ...
  for (const auto& graph_abc : table_12) {
    ...
    for (auto c_it = c_it_begin; c_it != c_it_end; ++c_it) {
      auto d = get<1>(*c_it);
      table_123.emplace_back(make_tuple(a, b, c, d));
    }
  }
  table_12.clear();

  for (const auto& graph_abcd : table_123) {
    auto a = get<0>(graph_abcd);
    auto b = get<1>(graph_abcd);
    auto c = get<2>(graph_abcd);
    auto d = get<3>(graph_abcd);
    auto d_it_begin = lower_bound
      (graph.begin(), graph.end(), d,
       &tuple_less_key<unsigned int, unsigned int>);  // quivalent to find in sorted list
    if (d_it_begin == graph.end()) {
      continue;
    }
    auto d_it_end = upper_bound
      (graph.begin(), graph.end(), d,
       &key_less_tuple<unsigned int, unsigned int>);  // quivalent to find in sorted list

    for (auto d_it = d_it_begin; d_it != d_it_end; ++d_it) {
      auto e = get<1>(*d_it);
      table_1234.emplace_back(make_tuple(a, b, c, d, e));
    }
  }
  table_123.clear();

  for (const auto& graph_abcde : table_1234) {
    auto a = get<0>(graph_abcde);
    auto b = get<1>(graph_abcde);
    auto c = get<2>(graph_abcde);
    auto d = get<3>(graph_abcde);
    auto e = get<4>(graph_abcde);
    auto e_it_begin = lower_bound
      (graph.begin(), graph.end(), e,
       &tuple_less_key<unsigned int, unsigned int>);  // quivalent to find in sorted list
    if (e_it_begin == graph.end()) {
      continue;
    }
    auto e_it_end = upper_bound
      (graph.begin(), graph.end(), e,
       &key_less_tuple<unsigned int, unsigned int>);  // quivalent to find in sorted list

    for (auto e_it = e_it_begin; e_it != e_it_end; ++e_it) {
      auto f = get<1>(*e_it);
      table_12345.emplace_back(make_tuple(a, b, c, d, e, f));
    }
  }
  table_1234.clear();

  for (const auto& graph_abcde : table_12345) {
    auto a = get<0>(graph_abcde);
    auto b = get<1>(graph_abcde);
    auto c = get<2>(graph_abcde);
    auto d = get<3>(graph_abcde);
    auto e = get<4>(graph_abcde);
    auto f = get<5>(graph_abcde);
    auto f_it_begin = lower_bound
      (graph.begin(), graph.end(), f,
       &tuple_less_key<unsigned int, unsigned int>);  // quivalent to find in sorted list
    if (f_it_begin == graph.end()) {
      continue;
    }
    auto f_it_end = upper_bound
      (graph.begin(), graph.end(), f,
       &key_less_tuple<unsigned int, unsigned int>);  // quivalent to find in sorted list

    for (auto f_it = f_it_begin; f_it != f_it_end; ++f_it) {
      auto g = get<1>(*f_it);
      data.emplace_back(make_tuple(a, b, c, d, e, f, g));
    }
  }
  table_12345.clear();
  ...
  for (int trial = 1; trial <= 30; ++trial) {
    ...
    //    cerr << "RW sketch size of table 123: " << rw_size << " / n: " << n << endl;
    rw_size = 0;
    ...
    for (auto& sketch_pair : rw_sketch) {
      ...
      for (unsigned int i = 0; i < sketch_a.table_123.size(); ++i) {
        const auto& sketch_t = sketch_a.table_123[i];
        auto d = get<3>(sketch_t);
        ...
        auto d_it_end = upper_bound
          (graph.begin(), graph.end(), d,
           &key_less_tuple<unsigned int, unsigned int>);
        if (d_it_begin == d_it_end) {
          continue;
        }

        if (sketch_a.was_complete) { // RANDOM WALK STAGE
          auto tuple_prob = sketch_a.prob[i];
          auto fan_out = d_it_end - d_it_begin;
          unsigned int idx = rand() % fan_out;
          auto d_it = d_it_begin + idx;
          sketch_a.add_table_1234(sketch_t, *d_it);
          sketch_a.temp_prob.emplace_back(tuple_prob / fan_out);
        } else {
          for (auto d_it = d_it_begin; d_it != d_it_end; ++ d_it) {
            sketch_a.add_table_1234(sketch_t, *d_it);
      ...
      rw_size += sketch_a.commit_table_1234();
    } // END FOR SKETCH_IT

    //    cerr << "RW sketch size of table 1234: " << rw_size << " / n: " << n << endl;
    rw_size = 0;
    ...
    for (auto& sketch_pair : rw_sketch) {
      ...
      for (unsigned int i = 0; i < sketch_a.table_1234.size(); ++i) {
        const auto& sketch_t = sketch_a.table_1234[i];
        auto e = get<4>(sketch_t);
        ...
        auto e_it_end = upper_bound
          (graph.begin(), graph.end(), e,
           &key_less_tuple<unsigned int, unsigned int>);
        if (e_it_begin == e_it_end) {
          continue;
        }

        if (sketch_a.was_complete) { // RANDOM WALK STAGE
          auto tuple_prob = sketch_a.prob[i];
          auto fan_out = e_it_end - e_it_begin;
          unsigned int idx = rand() % fan_out;
          auto e_it = e_it_begin + idx;
          sketch_a.add_table_12345(sketch_t, *e_it);
          sketch_a.temp_prob.emplace_back(tuple_prob / fan_out);
        } else {
          for (auto e_it = e_it_begin; e_it != e_it_end; ++ e_it) {
            sketch_a.add_table_12345(sketch_t, *e_it);
      ...
      rw_size += sketch_a.commit_table_12345();
    } // END FOR SKETCH_IT
    ...
    for (auto& sketch_pair : rw_sketch) {
      ...
      for (unsigned int i = 0; i < sketch_a.table_12345.size(); ++i) {
        const auto& sketch_t = sketch_a.table_12345[i];
        auto f = get<5>(sketch_t);
        ...
        auto f_it_end = upper_bound
          (graph.begin(), graph.end(), f,
           &key_less_tuple<unsigned int, unsigned int>);
        if (f_it_begin == f_it_end) {
          continue;
        }

        if (sketch_a.was_complete) { // RANDOM WALK STAGE
          auto tuple_prob = sketch_a.prob[i];
          auto fan_out = f_it_end - f_it_begin;
          unsigned int idx = rand() % fan_out;
          auto f_it = f_it_begin + idx;
          sketch_a.add_table_123456(sketch_t, *f_it);
          sketch_a.temp_prob.emplace_back(tuple_prob / fan_out);
        } else {
          for (auto f_it = f_it_begin; f_it != f_it_end; ++ f_it) {
            sketch_a.add_table_123456(sketch_t, *f_it);
      ...
      rw_size += sketch_a.commit_table_123456();
    } // END FOR SKETCH_IT


    cerr << "RW sketch size of table 123456: " << rw_size << " / n: " << n << endl;
