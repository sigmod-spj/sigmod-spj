...
typedef vector<tuple<unsigned int, unsigned int, unsigned int, unsigned int, unsigned int> > RELATION_TYPE;
...
struct sketch_unit {
  ...
  double estimate(int param) const {
    bool success = false;
    double p = 1.0;
    for (unsigned int i = 0; i < table_1234.size(); ++i) {
      if (tuple_filter(table_1234[i], param)) {
...
ostream& operator<<(ostream &os, sketch_unit s) {
  os << "MAX FREQUENCY: " << s.max_freq << endl
     ...
     << "TABLE 1234 SIZE: " << s.table_1234.size() << endl
     << "PROB: ";
...
int main(int argc, char* argv[]) {
  ...
  for (const auto& graph_abcd : table_123) {
    ...
    for (auto d_it = d_it_begin; d_it != d_it_end; ++d_it) {
      auto e = get<1>(*d_it);
      data.emplace_back(make_tuple(a, b, c, d, e));
    }
  }
  table_123.clear();
