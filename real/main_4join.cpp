...
typedef vector<tuple<unsigned int, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int> > RELATION_TYPE;
...
struct sketch_unit {
  ...
  double estimate(int param) const {
    bool success = false;
    double p = 1.0;
    for (unsigned int i = 0; i < table_12345.size(); ++i) {
      if (tuple_filter(table_12345[i], param)) {
...
int main(int argc, char* argv[]) {
  ...
  for (const auto& graph_abcde : table_1234) {
    ...
    for (auto e_it = e_it_begin; e_it != e_it_end; ++e_it) {
      auto f = get<1>(*e_it);
      data.emplace_back(make_tuple(a, b, c, d, e, f));
    }
  }
  table_1234.clear();
