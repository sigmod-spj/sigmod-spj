...
typedef std::vector<std::tuple<unsigned int, unsigned int> > RELATION_TYPE;
 ...
 * @param seed for randomness in each iteration
 * @return a filtered table that only contains tuples in table that passes the filter of i.
 */
RELATION_TYPE filter_table(const RELATION_TYPE& table,unsigned int i, unsigned seed) {
  RELATION_TYPE result;
  for (auto it = table.cbegin(); it != table.cend(); ++it) {
    if (tuple_filter(*it,i,seed)) {
...
int main(int argc, char* argv[]) {
  ...
  // Chooase appropriate synthetic dataset
  RELATION_TYPE data = load_data((argc>1)?argv[1]:"../data/uniform.dat");
   ...
   * Every 10 lines, the first line is the sample construction time for uds/wds/histo
   * The remaining 9 lines are the query time for uds/wds/histo/exact
   */
  ofstream time_file;
  ...
  for (int trial = 1; trial <= 30; ++trial) {
    ...
    cout << '.';
    duration<double> time_span_wds = time_span_solve
    ...
    cout << '.';
    duration<double> time_span_histo = time_span_solve_histo
      + duration_cast<duration<double>>(end - start);
    ...
    for (int param = 0; param < PARAM_UB; ++ param) {
      ...
      start = high_resolution_clock::now();
      RELATION_TYPE uds_sketch_filtered = filter_table(uds_sketch,param,seed);
      ...
      for (auto it = wds_sketch.cbegin(); it != wds_sketch.cend(); ++it) {
        ...
        RELATION_TYPE Ra_filtered = filter_table(Ra,param,seed);
      ...
      RELATION_TYPE filtered_data = filter_table(data,param,seed);
